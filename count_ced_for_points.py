    ...
    for method_name in predicted_points.keys():
        ...
        for img_name in predicted_points[method_name].keys():
            if img_name in gt_points:
                ...
                if os.path.isfile('{}/{}'.format(path_crops,img_name).replace('.pts', '.jpg')):
...
def main():
    ...
    ceds = count_ced(predicted_points, gt_points, f"{args.gt_path.replace('landmarks_task','landmarks_task')}/{args.dataset}/test")
...
#                          default=0.08)
#     args = parser.parse_args()
#     p = read_points(f'{args.gt_path}/{args.dataset}/test', args.max_points_to_read)
#     print(p)
